...
class tiffinScreen extends Component {
    ...
    checkLunchBox = (itemID) => {
        ...
        for (let i = 0; i < this.state.mealInfo.length; i++) {
            if (this.state.mealInfo[i].mealType === 'Lunch') lunch += this.state.mealInfo[i].amount;
        }
    ...
    checkDinnerBox = (itemID) => {
        ...
        let dinner = 0;
        for (let i = 0; i < this.state.mealInfo.length; i++) {
            if (this.state.mealInfo[i].mealType === 'Dinner') dinner += this.state.mealInfo[i].amount;
        }
    ...
    deleteMeal = (itemID) => {
        console.log(itemID);
        this.state.mealInfo.splice(itemID, 1);

        let temp = this.state.list;
        if (temp[itemID].checkDinner) {
            temp[itemID].checkDinner = false;
        }
        else {
            temp[itemID].checkLunch = false;
        }
        this.setState({ list: temp });
            ...
            <SafeAreaView style={styles.container}>
                <View style={{ flexDirection: 'row', backgroundColor: '#ffffe6' }} >
                    <View style={styles.headerStyle}>
                        ...
                        <Text style={{ marginTop: 38, marginLeft: 20, color: '#4d2800' }} h3>Tiffin-Service</Text>
                    </View>
